...
\begin{abstract}
\texttt{tree\_clustering.py} is a command-line batch tool that mirrors the
k-DPIs clustering workflow of the aliViz web application.
...
smallest group id, reroots and ladderizes the tree, runs automatic k-DPIs
...
\subsection{Purpose}
The script automates the same scientific workflow that a user would perform
interactively in aliViz:
\begin{enumerate}[nosep]
  \item load a phylogenetic tree;
  \item group sequences by a name field;
  \item reroot on a founder;
  \item ladderize;
  \item run k-DPIs Auto clustering;
...
\subsection{Relationship to aliViz}
Behaviour is intended to match aliViz where practical:
\begin{itemize}[nosep]
  \item same default grouping (delimiter \texttt{\_}, field~3);
  \item same k-DPIs bipartition objective
        ($\min\max(\mathrm{depth}_1,\mathrm{depth}_2)$)
        with both children required to meet minimum cluster size;
  \item same DPI estimator from mean pairwise tree-path distance;
  \item same cluster / noise naming convention (\texttt{\_cl-}$n$ / \texttt{\_cl-n});
  \item SVG layout inspired by aliViz linear AUTO / DPI export.
\end{itemize}
One intentional difference: founder selection uses the \emph{tree-path medoid}
of the group with the smallest group id (not Hamming distance on an alignment).
...
\texttt{--field} & $3$ &
  1-based field index used as the group label (aliViz Group dialog). \\
\texttt{--dsr} & $5\times10^{-5}$ &
...
# Match aliViz grouping and annotate FASTA
python3 tree_clustering.py trees_in/ trees_out/ \
  --delimiter _ --field 3 \
  --fasta-dir fasta_in/
...
\texttt{\_cl-}$n$, \texttt{\_cl-na}, or \texttt{\_cl-n} are stripped before
grouping.

Example (delimiter \texttt{\_}, field~$3$):
\begin{center}
\texttt{CAP008\_2000\_G1\_a} $\rightarrow$ group \texttt{G1}.
\end{center}
...
with headers ending in \texttt{\_cl-}$n$ or \texttt{\_cl-n} (noise).
...
\begin{enumerate}
  ...
  \item \textbf{k-DPIs Auto} clustering (Section~\ref{sec:kdpis}).
  ...
  \item \textbf{SVG export} with AUTO or DPI scale and per-cluster target lines
        (Section~\ref{sec:svg}).
\end{enumerate}
...
\begin{equation}
  ...
  \label{eq:dpi}
\end{equation}
This matches the aliViz tree-based DPI estimator
($\mathrm{dpi}=\overline{\delta}/(2\times\mathrm{dsr})$).

\subsection{Expected early tip distance}
Under a strict-clock star model,
\begin{equation}
  \mathrm{etd}=2\times\mathrm{dsr}\times\mathrm{DPI}.
  \label{eq:etd}
\end{equation}
Long-branch pre-noise removes tips whose incoming length exceeds $\mathrm{etd}$.
...
(``shallow subtrees first''), matching aliViz depth ladderize mode.
Tip display order (top to bottom) follows this child order after layout.

% -----------------------------------------------------------------------------
\section{k-DPIs Auto clustering}
\label{sec:kdpis}
% -----------------------------------------------------------------------------
...
\subsection{Bubble tree}
Clustering maintains a binary partition tree of tip sets (``k-DPIs leaves'').
...
\begin{itemize}[nosep]
  ...
  \item cluster $n\ge 1$: \texttt{\_cl-}$n$
\end{itemize}
...
\begin{enumerate}
  ...
  \item For each FASTA header, strip any old cluster suffix, look up the
        cluster id, and append \texttt{\_cl-}$n$ or \texttt{\_cl-n}.
...
An extra top spacer row holds the scale bar (analogous to aliViz placing the
scale on the reference row above the first tip).
...
\[
  ...
  \max_c\bigl(d(\mathrm{lca}_c)+\tau\bigr)\bigr)
\]
where the last term runs over non-noise clusters $c$ (so every cluster target
mark remains inside the plot). Then
...
\subsection{Per-cluster target lines}
For each non-noise cluster $c$ with tip set $T_c$ and
$L_c=\mathrm{lca}(T_c)$, draw a \emph{solid} vertical line at
\[
  x_c=5+\bigl(d(L_c)+\tau\bigr)\,x_{\mathrm{scale}}
\]
(clamped into the branch region if needed), spanning only the vertical extent
of $T_c$'s tip rows, coloured with the cluster palette colour of $c$.
No text annotation is attached to these lines.
Tip diamonds use half the historical aliViz size (half-diagonal $5$~px);
target lines use stroke width~$2$.
...
ladderized by depth, clustered with k-DPIs Auto, and drawn with per-cluster target
bars and a group/cluster legend.
...
\caption{Multi-cluster example
  ...
  k-DPIs Auto produced four non-noise clusters (\texttt{cl-4}) plus two noise tips:
  tip diamonds and solid target bars use palette colours~$1$--$4$;
  noise diamonds are grey.
  Each target bar sits at that cluster's $\mathrm{lca}+\tau$ and covers only
  ...
  \texttt{[Founder]}.
  Tip suffixes \texttt{\_cl-}$n$ / \texttt{\_cl-n} match optional FASTA annotation.}
\label{fig:ex-cl4}
\end{figure}
...
Parameter & Default \\
\midrule
Daily substitution rate (\texttt{dsr}) & $0.00005$ \\
...
Suppose $\mathrm{dsr}=5\times10^{-5}$ and filename DPI $=14$. Then
\[
  \tau=14\times5\times10^{-5}=7\times10^{-4},\qquad
  \mathrm{etd}=1.4\times10^{-3}.
\]
A tip set with mean pairwise path distance
$\overline{\delta}=2.1\times10^{-3}$ has
\[
  \widehat{\mathrm{DPI}}
  =\mathrm{round}\!\left(\frac{2.1\times10^{-3}}{2\times5\times10^{-5}}\right)
  =\mathrm{round}(21)=21>14,
\]
...
\texttt{aliViz.html} & Interactive reference UI \\
...
Defaults and SVG styling evolve with the aliViz / BioWidgets codebase.
