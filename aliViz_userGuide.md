...
**Load summary (`s`, `f`, `nf`, `dpi`, `dsr`, `etd`):** Above the alignment filename you will see a compact report, e.g. **`s=56, f=25, nf=31, dpi=24, dsr=0.00005, etd=0.0024`**, meaning 56 sequences total, 25 functional, 31 non-functional, **24** days post infection (DPI), daily substitution rate **dsr**, and **expected mean pairwise tip distance** **etd**.
...
- **etd (expected mean pairwise tip distance):** under an early-infection / strict-clock **star** approximation, mean tip-to-tip path length equals the tree diameter and is about twice the expected root→tip depth, so  
  **etd = 2 × dsr × DPI**  
  (e.g. DPI = 24 and dsr = 5×10⁻⁵ → etd = 0.0024 substitutions/site). This is an expected scale for the phylogeny, not a measured tree statistic.

The summary resets to **`s=0, f=0, nf=0, dpi=14, dsr=0.00005, etd=…`** when the alignment is cleared (or on page reload before a file is loaded).
...
- **DPI (days post infection):** Scales so an **expected** branch length lands at the **1/16 mark (32.5 px)** of the branch region. You supply two parameters:
  ...
  - **Mutations/day (MPD):** expected substitution rate per day. Seeded from the session **dsr** at alignment load (including after the on-load dsr dialog); default **0.00005**. You can still edit it in this dialog before export.
  - The expected branch length is **DPI × MPD** (e.g. 14 × 0.00005 = 0.0007), and the scale is set so this value maps to 32.5 px (i.e. `px per unit = 32.5 / (DPI × MPD)`). In circular mode the same value maps to a **16.25 px** radius (half, matching the Fixed circular convention).
...
- **Methods (selector):** **None**, **Hierarchical (tree-clade clustering)**, **Hierarchical (tree-cut clustering)**, **k-DPIs (divisive mean-distance clustering)**, **UMAP**, **MDS** (default).
...
### 4.3 k-DPIs (divisive mean-distance clustering)
- **Function:** Partition sample sequences by repeatedly splitting the cluster with the largest **mean pairwise tree path distance**. Bubble / legend **DPI** labels come from sequences (**mean** pairwise p-distance + JC69 + **dsr**).
...
- **Algorithm:**
  ...
  6. Non-noise leaves are numbered **1…k by phylogeny tip order**: clusters whose tips sit nearer the **top** of the tree (smaller row index after ladderize) get **lower** IDs; those toward the **bottom** get **higher** IDs. Undersized leaves are noise.
...
| Group          | Split name by delimiter; group = field value; unique IDs; dpi via mean pairwise p-distance + JC69: d=−(3/4)ln(1−4·api/3), dpi=d/(2×dsr) (dsr from Group dialog or on-load DPI prompt, default 5e-5 for the session); legend `label (n, dpi)`. |
